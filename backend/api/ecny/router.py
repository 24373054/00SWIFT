...
from database import EcnyTransaction, get_db
from ecny.auth_dep import require_ecny_cred, require_ecny_scope
...
from ecny.ledger import balance as ledger_balance
...
@router.post(
    ...
    dependencies=[Depends(require_ecny_scope("ecny.wallet"))],
)
...
def transfer_endpoint(body: TransferRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        check_limits(db, dst, body.amount_fen, incoming=True)
    except WalletError as e:
        raise HTTPException(400, str(e)) from e
    ...
    try:
        ...
        tx = transfer(db, src_acct, dst_acct, body.amount_fen, memo=body.memo or {"action": "p2p"})
        db.commit()
...
@router.post(
    "/issuance",
    response_model=LedgerTransactionResponse,
    dependencies=[Depends(require_ecny_scope("ecny.issuance"))],
)
def issuance_endpoint(body: IssuanceRequest, db: Session = Depends(get_db)):
    ...
    try:
        if body.action == "mint":
            ...
            tx_id = exchange_to_wallet(
                db, body.operator_account_id, body.wallet_id, body.amount_fen
            )
        elif body.action == "redeem_from_wallet":
            if not body.wallet_id:
                raise HTTPException(400, "wallet_id required for redeem_from_wallet")
            tx_id = redeem_from_wallet(
                db, body.wallet_id, body.operator_account_id, body.amount_fen
            )
        else:
...
@router.post(
    "/cross-border",
    response_model=BridgeTxResponse,
    dependencies=[Depends(require_ecny_scope("ecny.bridge"))],
)
def cross_border_endpoint(body: CrossBorderRequest, db: Session = Depends(get_db)):
    ...
    check_transaction(db, src, body.amount_fen, cross_border=True, counterparty=body.counterparty)
    try:
        src_acct = ledger_account_id_for(db, body.from_wallet_id)
        if channel == "mbridge":
            btx = settle_pvp(
            ...
            btx = settle_via_cips(
                db, src_acct, body.to_account_id, body.amount_fen, body.counterparty
            )
        db.commit()
        return BridgeTxResponse(
            uetr=btx.uetr,
            channel_id=btx.channel_id,
            from_currency=btx.from_currency,
            from_amount=btx.from_amount,
            to_currency=btx.to_currency,
            to_amount=btx.to_amount,
            fx_rate=btx.fx_rate,
            status=btx.status,
...
@router.get(
    ...
    dependencies=[Depends(require_ecny_scope("ecny.ledger"))],
)
def list_transactions(limit: int = 100, db: Session = Depends(get_db)):
    limit = max(1, min(limit, 500))
    rows = db.query(EcnyTransaction).order_by(EcnyTransaction.created_at.desc()).limit(limit).all()
    return [_tx_response(t) for t in rows]


@router.get(
    "/ledger/balance/{account_id}", dependencies=[Depends(require_ecny_scope("ecny.ledger"))]
)
...
@router.get("/bridge/channels", dependencies=[Depends(require_ecny_scope("ecny.bridge"))])
        ...
        "channels": [
            {
                ...
                "status": c.status,
                "fx_rate": json.loads(c.fx_rate or "{}"),
            }
            for c in rows
        ]
    }
...
@router.get(
    ...
    dependencies=[Depends(require_ecny_scope("ecny.compliance"))],
)
