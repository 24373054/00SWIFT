...
def validate_payment_instruction(db: Session, body: PaymentInstruction) -> PaymentInstructionValidation:
    validations = []
    ...
    validations.append(checks.check_account_format(db, body.creditor_account, body.creditor_agent_country))

    # 3. Instructed Amount (currency + amount + decimals).
    validations.append(checks.check_amount(body.instructed_amount.amount, body.instructed_amount.currency_code, db))
    ...
    validations.append(checks.check_country(body.debtor_agent_country, "Debtor_Agent_Country_Validation"))

    # 6. Creditor agent country.
    validations.append(checks.check_country(body.creditor_agent_country, "Creditor_Agent_Country_Validation"))
