...
from iso20022.ecny_messages import (
    build_ecny_cross_border_msg,
    build_ecny_issuance_msg,
    build_pacs002_ecny,
    build_pacs008_ecny,
)
...
def test_pacs008_ecny_has_wallet_and_purpose():
    xml = build_pacs008_ecny(
        debtor_wallet="w1", creditor_wallet="w2", amount_fen=10000, debtor_name="Alice"
    )
...
def test_cross_border_msg():
    xml = build_ecny_cross_border_msg(
        uetr="u1",
        channel="mbridge",
        from_currency="CNY",
        from_amount_fen=10000,
        to_currency="HKD",
        to_amount_fen=10800,
        fx_rate="1.08",
        counterparty="HKMA",
