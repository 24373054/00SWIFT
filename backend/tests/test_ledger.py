...
def test_mint_and_balance(db):
    get_or_create_account(db, "acct-op-1", "operator", "OP1")
    mint(db, "acct-op-1", 1_000_000)
    assert balance(db, "acct-op-1") == 1_000_000


def test_mint_rejects_non_operator(db):
    get_or_create_account(db, "acct-w-1", "wallet", "W1")
    with pytest.raises(LedgerError):
        mint(db, "acct-w-1", 100)


def test_transfer_atomic(db):
    get_or_create_account(db, "acct-a", "operator", "A")
    get_or_create_account(db, "acct-b", "operator", "B")
    mint(db, "acct-a", 500)
...
def test_exchange_atomic(db):
    ...
    get_or_create_account(db, "acct-cny", "operator", "C", "CNY")
    hkd = EcnyAccount(
        account_id="acct-hkd", owner_type="operator", owner_ref="H", currency="HKD", balance=0
    )
    db.add(hkd)
    db.flush()
...
def test_mint_has_balanced_liability_entries(db):
    from database import EcnyEntry

    get_or_create_account(db, "acct-op-balanced", "operator", "OP")
    tx = mint(db, "acct-op-balanced", 1_000)
    entries = db.query(EcnyEntry).filter(EcnyEntry.tx_id == tx.tx_id).all()
    assert len(entries) == 2
    assert sum(e.amount for e in entries if e.direction == "debit") == 1_000
    assert sum(e.amount for e in entries if e.direction == "credit") == 1_000
