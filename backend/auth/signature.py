...
from core.errors import SEVERITY_FATAL, SwiftApiException, missing_signature, invalid_token
...
def verify_swift_signature(
    ...
    try:
        ...
        raise SwiftApiException(
            "SwAP502", SEVERITY_FATAL, f"Signature verification failed: {exc}", 401
        )

    # sub must match the credential's cert_subject.
    if claims.get("sub") != cred.cert_subject:
        raise SwiftApiException(
            "SwAP502", SEVERITY_FATAL,
