...
def _pacs008_pkg_dir() -> Optional[str]:
    try:
...
def _xsd_path_for(version: str) -> Optional[str]:
...
def validate_xml(xml_str: str, version: str) -> Tuple[bool, Optional[str]]:
    ...
    if schema.validate(doc):
        return True, None
    return False, " ; ".join(
        f"{err.line}: {err.message}" for err in schema.error_log[:5]
    )


def validate_pacs008(xml_str: str) -> Tuple[bool, Optional[str]]:
