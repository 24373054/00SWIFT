...
from core.errors import register_exception_handlers, SwiftApiException, malformed_request
from core.middleware import RequestContextMiddleware
from database import SessionLocal, init_db, seed_reference_data, seed_ecny_data
...
async def lifespan(app: FastAPI):
    ...
    if settings.is_live:
        ...
        print("⚠️  SWIFT_ENV=live — production mode active. All write ops default to dry_run.")
    elif settings.swift_env == "pilot":
        settings.validate_for_pilot()
        print("SWIFT_ENV=pilot — real SWIFT sandbox hosts.")

    init_db()
    seed_reference_data()
    seed_ecny_data()
    print(f"SWIFT Dev Sandbox started in '{settings.swift_env}' mode.")
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost", "http://127.0.0.1", "http://localhost:8765", "http://127.0.0.1:8765"],
    allow_credentials=False,  # credentials=True with wildcard origins is invalid
    allow_methods=["GET", "POST", "PATCH", "DELETE", "OPTIONS"],
    allow_headers=["Authorization", "Content-Type", "Accept", "X-Request-ID", "x-bic", "X-SWIFT-Signature", "X-API-Minor-Version"],
)
...
# Business routers — mounted incrementally in stages B-E. Each is imported
# lazily so the app still boots if a stage's module isn't written yet.
try:
    from api.swiftref.router import router as swiftref_router
    app.include_router(swiftref_router)
except ImportError:
    pass

try:
    from api.preval.router import router as preval_router
    app.include_router(preval_router)
except ImportError:
    pass

try:
    from api.gpi.router import router as gpi_router
    app.include_router(gpi_router)
except ImportError:
    pass
...
# e-CNY router — digital yuan cross-border payment system.
try:
    from api.ecny.router import router as ecny_router
    app.include_router(ecny_router)
except ImportError:
    pass

# Internal e-CNY admin endpoints (operator/wallet provisioning, dashboard stats).
try:
    from admin.ecny_admin import router as ecny_admin_router
    app.include_router(ecny_admin_router)
except ImportError:
    pass

# Internal /api/credentials (admin-token gated, auto-provisions mock PKI certs).
...
def dashboard_stats(_: None = Depends(require_admin_token)):
    ...
    from database import AppCredential, OAuthToken, ApiRequest, PaymentState
    db = SessionLocal()
    try:
        ...
        for p in db.query(PaymentState).all():
            payment_by_state[p.transaction_status] = payment_by_state.get(p.transaction_status, 0) + 1
...
def health():
    ...
    return {"status": "ok", "env": get_settings().swift_env}
