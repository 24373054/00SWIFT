...
from database import EcnyAccount, EcnyTransaction, EcnyEntry
...
def get_or_create_account(db: Session, account_id: str, owner_type: str,
                          owner_ref: str, currency: str = "CNY") -> EcnyAccount:
    acct = db.query(EcnyAccount).filter(EcnyAccount.account_id == account_id).first()
    if acct:
        return acct
    acct = EcnyAccount(account_id=account_id, owner_type=owner_type,
...
def _apply_transfer(db: Session, tx: EcnyTransaction, from_acct: EcnyAccount,
                    to_acct: EcnyAccount, amount: int) -> None:
    ...
    db.add(EcnyEntry(entry_id=_new_id("ent"), tx_id=tx.tx_id,
                     account_id=from_acct.account_id, direction="debit", amount=amount))
    db.add(EcnyEntry(entry_id=_new_id("ent"), tx_id=tx.tx_id,
                     account_id=to_acct.account_id, direction="credit", amount=amount))
    db.flush()
...
def mint(db: Session, to_account_id: str, amount: int,
         memo: Optional[Dict[str, Any]] = None) -> EcnyTransaction:
    ...
    tx = EcnyTransaction(tx_id=_new_id("tx"), tx_type="mint", status="pending",
                         amount=amount, currency=to_acct.currency,
                         to_account=to_account_id, memo=json.dumps(memo or {}))
    db.add(tx)
    db.flush()
    to_acct.balance += amount
    db.add(EcnyEntry(entry_id=_new_id("ent"), tx_id=tx.tx_id,
                     account_id=to_account_id, direction="credit", amount=amount))
    db.flush()
    return _settle(db, tx)


def burn(db: Session, from_account_id: str, amount: int,
         memo: Optional[Dict[str, Any]] = None) -> EcnyTransaction:
    ...
    tx = EcnyTransaction(tx_id=_new_id("tx"), tx_type="burn", status="pending",
                         amount=amount, currency=from_acct.currency,
                         from_account=from_account_id, memo=json.dumps(memo or {}))
    db.add(tx)
    db.flush()
    from_acct.balance -= amount
    db.add(EcnyEntry(entry_id=_new_id("ent"), tx_id=tx.tx_id,
                     account_id=from_account_id, direction="debit", amount=amount))
    db.flush()
    return _settle(db, tx)


def transfer(db: Session, from_account_id: str, to_account_id: str, amount: int,
             memo: Optional[Dict[str, Any]] = None) -> EcnyTransaction:
    ...
    tx = EcnyTransaction(tx_id=_new_id("tx"), tx_type="transfer", status="pending",
                         amount=amount, currency=from_acct.currency,
...
def exchange(db: Session, from_account_id: str, to_account_id: str,
             from_amount: int, to_amount: int,
             memo: Optional[Dict[str, Any]] = None) -> EcnyTransaction:
    ...
    tx = EcnyTransaction(tx_id=_new_id("tx"), tx_type="exchange", status="pending",
                         amount=from_amount, currency=from_acct.currency,
                         from_account=from_account_id, to_account=to_account_id,
    ...
    db.add(EcnyEntry(entry_id=_new_id("ent"), tx_id=tx.tx_id,
                     account_id=from_account_id, direction="debit", amount=from_amount))
    db.add(EcnyEntry(entry_id=_new_id("ent"), tx_id=tx.tx_id,
                     account_id=to_account_id, direction="credit", amount=to_amount))
    return _settle(db, tx)


def transaction(db: Session, tx_id: str) -> Optional[EcnyTransaction]:
